...
int Mesh::load(std::istream &in)
{
  return read_obj(in);
}

int Mesh::read_obj(std::istream & f)
{
  std::string line;
  std::string vTok("v");
  std::string fTok("f");
  ...
  // std::cout<<"Num Triangles: "<< numTriangles <<"\n";
  return numTriangles;
}
...
int Mesh::load_mesh(const char * filename, bool normalize)
{
  std::ifstream f ;
  f.open(filename);
  if(!f.good()) {
